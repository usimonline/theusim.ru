...
$REQUEST_URI_2 = $REQUEST_URI;
$Request_per = false;
switch($REQUEST_URI ){ //для разделов
	case '/'.$razdel_vitina_1.'/':
		$Request_per = true;//нужно для разделов
		$REQUEST_URI = '/pastnews/'.$razdel_vitina_1.'/50/';
		$title_temp ='Изменить имидж';
		$desc_temp = 'Консультация по изменению имиджа';
		$key_temp_head = $desc_temp;
		$keys_rub = 'Консультация Personal Image';
		$url_og_rub = '/'.$razdel_vitina_1.'/';
		$url_og_picture_rub = "/index/111111.webp";
		break;
	case '/': $REQUEST_URI = '/pastnews/'.$razdel_vitina_0.'/50/';
		$Request_per = true;//нужно для разделов
		$title_temp ='Главная';
		$desc_temp = 'Консультация имиджмейкера и Преображение';
		$key_temp_head = $desc_temp;
	$keys_rub = 'Консультация Personal Image';
	$url_og_rub = '/';
	$url_og_picture_rub = "/index/111111.webp";
		break;
	case '/'.$razdel_vitina_8.'/': $REQUEST_URI = '/pastnews/'.$razdel_vitina_8.'/50/';
		$Request_per = true;//нужно для разделов
		$title_temp ='Обучение';
		$desc_temp = 'Обучение, курсы стилиста, курсы визажиста, подбор одежды';
		$key_temp_head = $desc_temp;
		$keys_rub = 'Консультация Personal Image';
		$url_og_rub = '/'.$razdel_vitina_8.'/';
		$url_og_picture_rub = "/index/111111.webp";
		break;
	case '/'.$razdel_vitina_1.'/'.$razdel_vitina_2.'/': $REQUEST_URI = '/pastnews/'.$razdel_vitina_2.'/50/';
		$Request_per = true;//нужно для разделов
		$title_temp ='Консультация';
		$desc_temp = 'Консультация имиджмейкера для мужчин и женщин';
		$key_temp_head = $desc_temp;
		$keys_rub = 'Консультация Personal Image';
		$url_og_rub = '/'.$razdel_vitina_2.'/';
		$url_og_picture_rub = "/index/111111.webp";
		break;
	case '/'.$razdel_vitina_1.'/'.$razdel_vitina_3.'/': $REQUEST_URI = '/pastnews/'.$razdel_vitina_3.'/50/';
		$Request_per = true;//нужно для разделов
		$title_temp ='Макияж';
		$desc_temp = 'Урок макияжа и палитра';
		$key_temp_head = $desc_temp;
		$keys_rub = 'Консультация Personal Image';
		$url_og_rub = '/'.$razdel_vitina_3.'/';
		$url_og_picture_rub = "/index/111111.webp";
		break;
	case '/'.$razdel_vitina_1.'/'.$razdel_vitina_4.'/': $REQUEST_URI = '/pastnews/'.$razdel_vitina_4.'/50/';
		$Request_per = true;//нужно для разделов
	$title_temp ='Изменить прическу';
	$desc_temp = 'Сопровождение в салон красоты и подбор прически';
	$key_temp_head = $desc_temp;
	$keys_rub = 'Консультация Personal Image';
	$url_og_rub = '/'.$razdel_vitina_4.'/';
	$url_og_picture_rub = "/index/111111.webp";
	break;
	case '/'.$razdel_vitina_1.'/'.$razdel_vitina_5.'/': $REQUEST_URI = '/pastnews/'.$razdel_vitina_5.'/50/';
		$Request_per = true;//нужно для разделов
	$title_temp ='Гардероб';
	$desc_temp = 'Разбор гардероба и лукбук одежды';
	$key_temp_head = $desc_temp;
	$keys_rub = 'Консультация Personal Image';
	$url_og_rub = '/'.$razdel_vitina_5.'/';
	$url_og_picture_rub = "/index/111111.webp";
	break;
	case '/'.$razdel_vitina_1.'/'.$razdel_vitina_6.'/': $REQUEST_URI = '/pastnews/'.$razdel_vitina_6.'/50/';
		$Request_per = true;//нужно для разделов
	$title_temp ='Палитры и брендбуки ';
	$desc_temp = 'Палитра цветов для одежды и электронный брендбук';
	$key_temp_head = $desc_temp;
	$keys_rub = 'Консультация Personal Image';
	$url_og_rub = '/'.$razdel_vitina_6.'/';
	$url_og_picture_rub = "/index/111111.webp";
	break;
	case '/'.$razdel_vitina_1.'/'.$razdel_vitina_7.'/': $REQUEST_URI = '/pastnews/'.$razdel_vitina_7.'/50/';
		$Request_per = true;//нужно для разделов
	$title_temp ='Шоппинг с имиджмейкером';
	$desc_temp = 'Шоппинг с имиджмейкером в Минске и за границей';
	$key_temp_head = $desc_temp;
	$keys_rub = 'Консультация Personal Image';
	$url_og_rub = '/'.$razdel_vitina_7.'/';
	$url_og_picture_rub = "/index/111111.webp";
	break;
}
...
if ($Request_per) {
	$select = "SELECT * FROM $Name_database.$table WHERE url = '$REQUEST_URI_2' ";
...
$route = false;
if (empty($row) or $Request_per) {
	$route = true; // загружается l-main
}
	if ($row['datetime'] == NULL) $page['datetime'] = '';
	else $page['datetime'] = $row['datetime'];
	if ($row['teme'] == NULL) $page['teme'] = '';
	else $page['teme'] = $row['teme'];
	if ($row['description'] == NULL) $page['description'] = '';
	else $page['description'] = $row['description'];
	if ($row['comments'] == NULL) $page['comments'] = '';
	else $page['comments'] = $row['comments'];
	if ($row['url'] == NULL) $page['url'] = '';
	else $page['url'] = $row['url'];
	if ($row['text'] == NULL) $page['text'] = '';
	else $page['text'] = $row['text'];
	if ($row['keys'] == NULL) $page['keys'] = '';
	else $page['keys'] = $row['keys'];
	if ($row['id'] == NULL) $page['id'] = '';
	else $page['id'] = $row['id'];
...
else $page['url_frame'] = $row['url_frame'];
...
if ($route){
	switch($REQUEST_URI){
		...
		default:
			...
			$rubrika_2_array = Array('empty', $razdel_vitina_0, $razdel_vitina_1, $razdel_vitina_2, $razdel_vitina_3, $razdel_vitina_4,
				$razdel_vitina_5, $razdel_vitina_6, $razdel_vitina_7, $razdel_vitina_8);
